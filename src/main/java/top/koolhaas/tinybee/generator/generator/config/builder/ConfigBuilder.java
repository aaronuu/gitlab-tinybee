package top.koolhaas.tinybee.generator.generator.config.builder;
...
import top.koolhaas.tinybee.generator.generator.config.po.TableFill;
...
public class ConfigBuilder {
    ...
    public ConfigBuilder(PackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig,
                         TemplateConfig template, GlobalConfig globalConfig, TransferCodeConfig transferCodeConfig) {
        // 全局配置
        ...
        this.transferCodeConfig = transferCodeConfig;
        //SQLITE 数据库不支持注释获取
    ...
    public TransferCodeConfig getTransferCodeConfig() {
        return transferCodeConfig;
    }
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.DOMAIN, joinPackage(config.getParent(), config.getDomain()));
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getDomain(), outputDir, ConstVal.DOMAIN_PATH, ConstVal.DOMAIN);
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            if (StrUtil.isNotEmpty(globalConfig.getEntityName())) {
            ...
            if (StrUtil.isNotEmpty(tableInfo.getDomainType()) && StrUtil.isNotEmpty(globalConfig.getDomainName())) {
                tableInfo.setServiceAggName(String.format(tableInfo.getDomainType(), entityName));
            }
    ...
    private void checkImportPackages(TableInfo tableInfo) {
        if (StrUtil.isNotEmpty(strategyConfig.getSuperEntityClass())) {
            // 自定义父类
        ...
        if (StrUtil.isNotEmpty(strategyConfig.getVersionFieldName())) {
                ...
                while (results.next()) {
                    ...
                    if (StrUtil.isNotEmpty(tableName)) {
                        ...
                        if (commentSupported) {
                            ...
                            if (org.apache.commons.lang3.StringUtils.containsIgnoreCase(tableComment, "Aggregate")) {
                                tableInfo.setDomainType("%sAggregateService");
                            }
                        }
    ...
    private boolean tableNameMatches(String setTableName, String dbTableName) {
        return setTableName.equalsIgnoreCase(dbTableName)
                || BaseUtil.matches(setTableName, dbTableName);
    }
                ...
                while (results.next()) {
                    ...
                    if (DbType.H2 == dbType) {
                        ...
                        if (DbType.DB2 == dbType || DbType.SQLITE == dbType) {
                            isId = StrUtil.isNotEmpty(key) && "1".equals(key);
                        } else {
                            isId = StrUtil.isNotEmpty(key) && "PRI".equals(key.toUpperCase());
                        }
                    }
    ...
    private String joinPath(String parentDir, String packageName) {
        ...
        if (!BaseUtil.endsWith(parentDir, File.separator)) {
